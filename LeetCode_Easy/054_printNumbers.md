...
### 分析
 - 执行用时 :20 ms, 在所有 C++ 提交中击败了16.81%的用户内存消耗 :11.4 MB, 在所有 C++ 提交中击败了100.00%的用户。时间上不是很理想。也是看到
   提交结果后意识到也许可以用递归来做节省时间。题目本身还是很简单的，一看就懂。
 - 而有一解法使用了std::generate，其为STL中的内容，可作了解。http://c.biancheng.net/view/620.html
